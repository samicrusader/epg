...
for item in listings.keys():
    for listing in listings[item]['listings']:
        programme = f'    <programme start="{datetime.fromtimestamp(listing["broadCastStartDate"]).strftime("%Y%m%d%H%M%S")} +0900" stop="{datetime.fromtimestamp(listing["broadCastEndDate"]).strftime("%Y%m%d%H%M%S")} +0900" channel="{item}">\n'
